 ...
 * @Date: 2021-06-01 12:03:37 
 * @Last Modified by: Xin 
 * @Last Modified time: 2021-06-01 20:30:19
 */
...
const XHY_API_HOST = 'http://120.79.242.198'
...
async function task_list() {
  ...
  await task_list_API()
//   console.log(result);
  let TaskList = result.data.taskDetails
  TaskList.forEach((item)=>{
    //   console.log(item);
      info = item.goldDailyTaskSetting
      console.log(`[${info.id}]${info.name}(${info.frequency+"/"+info.number})可获得:${info.goldNum}💰`);
  })
}
...
async function task_list_API() {
  await getRequest(`lucky/v1/gold/dailyTask?deviceType=2`)
}
...
function postRequest(url, timeout = 1000){
  return new Promise(resolve => {
    setTimeout(() => {
